...
# Application
APP_NAME="JMXMiniProbe"
APP_LONG_NAME="JMXMiniProbe"
...
# description: JMXMiniProbe
 
# Initialization block for the install_initd and remove_initd scripts used by
#  SUSE linux distributions.
### BEGIN INIT INFO
# Provides: JMXMiniProbe
# Required-Start: $local_fs $network $syslog
# Should-Start: 
# Required-Stop:
# Default-Start: 2 3 4 5
# Default-Stop: 0 1 6
# Short-Description: JMXMiniProbe
# Description: Paessler PRTG Java JMX MiniProbe
### END INIT INFO
...
checkUser() {
    ...
    then
        ...
        then
            eval echo `gettext 'Could not run the command using user \"$RUN_AS_USER\".'`
            eval echo `gettext 'Advice: Make sure the user \"$RUN_AS_USER\" has a shell.'`
            eval echo `gettext '        If user \"$RUN_AS_USER\" has a no shell, you can specify one using SU_OPTS if your platform support it.'`
            eval echo `gettext '        For example, at the top of this script you can set: SU_OPTS=\"-s /bin/bash\".'`
            eval echo `gettext '        Another workaround would be to use a OS service management tool if available on your platform.'`
            eval echo `gettext '        OS service management tools supported by this script are described at the top of this script.'`
        fi
...
systemdStop() {
    ...
    if [ $? -ne 0 ] ; then
        eval echo `gettext 'Failed to stop $APP_NAME'`
        exit 1
    fi
}
...
systemdRestart() {
    ...
    if [ $? -ne 0 ] ; then
        eval echo `gettext 'Failed to restart $APP_NAME'`
        exit 1
    fi
